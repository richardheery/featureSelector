...
separate_training_and_test_data = function(data, k_folds_indices, test_fold){
  ...
  if(!all(sort(unlist(k_folds_indices)) == seq.int(nrow(data)))){
    stop("Indices in k_folds_indices should cover all rows in data and each index sould occur in only a single fold")
  }
...
#' @return 
#' @export
cross_validated_subset_selection = function(data, response, K = 10){
